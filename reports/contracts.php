...
<?php 
   ...
   function Header()
   {
    ...
    $this->Cell(50);
    // Title
    $this->Cell(90,10,'Freelancer Contracts Report',1,0,'C');
    // Line break
    $this->Ln(20);
   }
   ...
   function FancyTable($header)
   {
    ...
    while($row = mysqli_fetch_array($GLOBALS['retval'])){
        $this->Cell($w[0],6,getAuthor($row['freelancer_id'], $GLOBALS['conn'])['full_name'],'LR',0,'L',$fill);
        $this->Cell(120,6,getJobDetail($row['job_id'], $GLOBALS['conn'])['title'],'LR',0,'L',$fill);
        $this->Ln();
        $fill = !$fill;
    }
